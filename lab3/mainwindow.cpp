...
void DrawArea::paintEvent(QPaintEvent* event){
    ...
    //https://forum.qt.io/topic/108286/how-to-draw-qgraphicspolygonitem/14
    QPolygonF bird;
    ...
    QBrush waterBrush = QBrush(QColor(3,119,252));
    QBrush greyBrush = QBrush(QColor(62,62,62));
    QBrush greenBrush = QBrush(QColor(64, 179, 77));
    ...
    for (auto i = flashList.begin(); i != flashList.end();) {
        (*i)(painter, 0);
        if ((*i).state() == 0) {
            i = flashList.erase(i);
        } else {
    ...
    for (auto i = mirrorFlashList.begin(); i != mirrorFlashList.end();) {
        (*i)(painter, 0);
        if ((*i).state() == 0) {
            i = mirrorFlashList.erase(i);
        } else {
            ++i;
        }
    }
